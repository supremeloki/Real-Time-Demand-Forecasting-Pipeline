...
snapp_data_path = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(__file__))),
    "data_artifacts",
    "tehran_traffic_simulator",
    "generate_snapp_data.py",
)
...
class BatchFeatureCreator:
    ...
    def _load_raw_data(self):
        if (
            ...
            and "synthetic_snapp_data" in self.synthetic_data_path
        ):
    ...
    def _calculate_grid_ids(self, df):
        df["grid_id_latitude"] = (
            df["pickup_latitude"] / (self.grid_size_km / 111.0)
        ).astype(int)
        ...
        df["grid_id_longitude"] = (
            df["pickup_longitude"] / (self.grid_size_km / (111.0 * df["cos_lat"]))
        ).astype(int)
    ...
    def _aggregate_time_series_features(self, df):
        ...
        grouped = (
            df_agg.groupby(["grid_id_latitude", "grid_id_longitude", "timestamp_15min"])
            .agg({"demand_at_pickup": ["count", "sum"]})
            .reset_index()
        )
        ...
        grouped["feature_window_end"] = grouped["event_timestamp"] + pd.Timedelta(
            minutes=15
        )
        ...
        grouped["peak_hour_indicator"] = (
            (grouped["hour_of_day"] >= 7) & (grouped["hour_of_day"] <= 9)
        ) | ((grouped["hour_of_day"] >= 17) & (grouped["hour_of_day"] <= 20)).astype(
            int
        )

        # Sort for rolling calculations
        grouped = grouped.sort_values(
            ["grid_id_latitude", "grid_id_longitude", "event_timestamp"]
        )
        ...
        for window_size in self.lookback_windows_min:
            ...
            grouped[f"demand_last_{window_size}min"] = grouped.groupby(
                ["grid_id_latitude", "grid_id_longitude"]
            )["demand_sum"].transform(
                lambda x: x.expanding(window_periods).sum().shift(1).fillna(0)
            )

        # Target: next period demand
        grouped[self.target_variable] = (
            grouped.groupby(["grid_id_latitude", "grid_id_longitude"])["demand_sum"]
            .shift(-1)
            .fillna(0)
        )
    ...
    def generate_batch_features(self):
        ...
        final_features_df = final_features_df[
            final_features_df[self.target_variable] > 0
        ]
