...
class DynamicResourceGuardian:
    ...
    def __init__(
        ...
        logging.info(
            f"DynamicResourceGuardian initialized. Min={min_instances}, Max={max_instances} instances."
        )
    ...
    def _get_current_metrics(self) -> Dict[str, Any]:
        ...
        simulated_qps = self.current_instances * random.uniform(
            50, 150
        )  # QPS varies per instance
        ...
        logging.debug(
            f"Current Metrics: CPU={simulated_cpu_util:.2f}, QPS={simulated_qps:.0f}"
        )
        return {
            "cpu_utilization": simulated_cpu_util,
            "queries_per_second": simulated_qps,
        }
    ...
    def _calculate_target_instances(self, metrics: Dict[str, Any]) -> int:
        ...
        target_by_cpu = int(
            ...
            * (metrics["cpu_utilization"] / self.target_cpu_utilization)
        )
        target_by_qps = int(
            metrics["queries_per_second"] / self.target_qps_per_instance
        )
        ...
        desired_instances = max(
            self.min_instances, min(self.max_instances, desired_instances)
        )
    ...
    def manage_resources(self) -> Dict[str, Any]:
        ...
        if (current_time - self.last_scale_action) < self.cooldown_period:
            logging.info(
                f"Cooldown period active. Skipping scaling for {current_time - self.last_scale_action} (Remaining: {self.cooldown_period - (current_time - self.last_scale_action)})"
            )
            return {
                "action": "NO_ACTION",
        ...
        if desired_instances > self.current_instances:
            ...
            logging.warning(
                f"Scaling up from {self.current_instances} to {desired_instances} instances. Reason: {reason}"
            )
            ...
            logging.warning(
                f"Scaling down from {self.current_instances} to {desired_instances} instances. Reason: {reason}"
            )
            self.current_instances = desired_instances
            self.last_scale_action = current_time
        else:
            logging.info(
                f"Current instances {self.current_instances} is optimal. Reason: {reason}"
            )
...
if __name__ == "__main__":
    guardian = DynamicResourceGuardian(
        min_instances=2, max_instances=10, cooldown_period_minutes=1
    )
    ...
    for i in range(15):  # Simulate 15 monitoring cycles
        ...
        print(
            f"  Action: {status['action']}, Instances: {status['current_instances']} (Desired: {status['desired_instances']})"
        )
        print(f"  Reason: {status['reason']}")
        print(
            f"  CPU Util: {status['metrics']['cpu_utilization']:.2f}, QPS: {status['metrics']['queries_per_second']:.0f}"
        )
        import time
