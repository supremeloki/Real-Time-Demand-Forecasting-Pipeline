...
class DataSanitizer:
    def __init__(
        self, imputation_strategy: str = "mean", outlier_threshold: float = 3.0
    ):
    ...
    def cleanse_data(
        self, data_frame: pd.DataFrame, numerical_columns: List[str] | None = None
    ) -> pd.DataFrame:
        cleaned_df = data_frame.copy()
        for col in (
            numerical_columns
            if numerical_columns
            else cleaned_df.select_dtypes(include=np.number).columns
        ):
