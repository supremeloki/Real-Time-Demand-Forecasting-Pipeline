...
class EventAnomalySimulator:
    ...
    def __init__(
        self,
        kafka_producer=None,
        input_topic: str = "snapp.ride_events_raw",
        geohash_pool: List[str] | None = None,
    ):
        self.kafka_producer = (
            kafka_producer  # Expects an instance of SnappKafkaProducer
        )
        self.input_topic = input_topic
        self.geohash_pool = (
            geohash_pool
            if geohash_pool
            else ["dr5ru", "dr5ry", "dr5rz", "dr5r7", "dr5rg"]
        )
    ...
    def _generate_normal_event(
        self, geohash_id: str, timestamp: datetime
    ) -> Dict[str, Any]:
    ...
    def _generate_spike_anomaly_event(
        self, geohash_id: str, timestamp: datetime, spike_factor: float = 5.0
    ) -> Dict[str, Any]:
        """Generates a ride event with a sudden spike in requests."""
        request_count = max(
            self.normal_base_request_count * spike_factor,
            round(random.gauss(self.normal_base_request_count * spike_factor, 10)),
        )
        logging.warning(
            f"Injecting spike anomaly in {geohash_id} at {timestamp}: {request_count} requests."
        )
        return {
            "geohash_id": geohash_id,
    ...
    def _generate_drop_anomaly_event(
        self, geohash_id: str, timestamp: datetime, drop_factor: float = 0.1
    ) -> Dict[str, Any]:
        """Generates a ride event with a sudden drop in requests (near zero)."""
        request_count = max(
            0, round(random.gauss(self.normal_base_request_count * drop_factor, 1))
        )
        logging.warning(
            f"Injecting drop anomaly in {geohash_id} at {timestamp}: {request_count} requests."
        )
        return {
            "geohash_id": geohash_id,
            "timestamp": timestamp.isoformat() + "Z",
    ...
    def simulate_events(
        ...
        """
        logging.info(
            f"Starting event simulation for {num_events} events. Anomaly frequency: {anomaly_freq}"
        )
        ...
        for i in range(num_events):
            ...
            if self.kafka_producer:
                try:
                    ...
                    logging.debug(
                        f"Sent event to Kafka: {event_data['event_type']} for {geohash}"
                    )
...
if __name__ == "__main__":
    ...
    class MockKafkaProducer:
        ...
        def flush(self):
            logging.info(
                f"MockKafkaProducer flushed {len(self.sent_messages)} messages."
            )
