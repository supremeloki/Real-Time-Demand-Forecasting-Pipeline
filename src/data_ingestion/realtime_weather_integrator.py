...
class RealtimeWeatherIntegrator:
    ...
    """
    def __init__(self, api_key: str = "DUMMY_API_KEY", base_url: str = "https://api.weather.com/v1/forecast/hourly/10day.json"):
        self.api_key = api_key
        self.base_url = base_url
        self.last_fetch_time: Dict[str, datetime] = {} # Cache last fetch time per location
        self.cache_duration = timedelta(minutes=15) # Only refetch if older than 15 mins
    ...
    def _fetch_weather_from_api(self, lat: float, lon: float) -> Dict[str, Any]:
        ...
        precipitation = random.choices([0.0, 0.1, 0.5, 2.0, 5.0], weights=[0.7, 0.2, 0.05, 0.04, 0.01])[0]
        weather_condition = random.choice(["Clear", "Cloudy", "Partly Cloudy", "Rain", "Fog"])
    ...
    def get_weather_data(self, geohash_id: str) -> Dict[str, Any]:
        ...
        # Check cache
        if geohash_id in self.weather_cache and \
           current_time - self.last_fetch_time.get(geohash_id, datetime.min) < self.cache_duration:
...
if __name__ == '__main__':
    ...
    print(f"Cached Weather for dr5ru: {weather_cached['temperature_celsius']}°C, {weather_cached['weather_condition']}")
    ...
    weather_integrator.last_fetch_time["dr5ru"] = datetime.now() - timedelta(minutes=20) # Force cache invalidation

    weather_fresh = weather_integrator.get_weather_data("dr5ru")
    print(f"Fresh Weather for dr5ru: {weather_fresh['temperature_celsius']}°C, {weather_fresh['weather_condition']}")