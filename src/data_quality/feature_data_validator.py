                ...
                if expected_type:
                    if not isinstance(value, expected_type):
                        if isinstance(expected_type, tuple):
                            ...
                            errors.append(
                                f"Invalid type for '{feature_name}': Expected one of ({expected_type_names}), got {type(value).__name__}"
                            )
                        else:
                            errors.append(
                                f"Invalid type for '{feature_name}': Expected {expected_type.__name__}, got {type(value).__name__}"
                            )
                        type_check_failed = True

                if (
                    not type_check_failed
                ):  # Only proceed with numerical/allowed value checks if type is correct
                    # Check min_value
                    min_value = rules.get("min_value")
                    if min_value is not None and value < min_value:
                        errors.append(
                            f"Value for '{feature_name}' ({value}) is below min_value ({min_value})"
                        )
                    ...
                    if max_value is not None and value > max_value:
                        errors.append(
                            f"Value for '{feature_name}' ({value}) is above max_value ({max_value})"
                        )
                    ...
                    if allowed_values is not None and value not in allowed_values:
                        errors.append(
                            f"Value for '{feature_name}' ({value}) is not in allowed_values: {allowed_values}"
                        )
...
if __name__ == "__main__":
    ...
    missing_features = {"hour_of_day": 12, "demand_current_5min": 10.0}
