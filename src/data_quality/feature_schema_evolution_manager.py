...
class FeatureSchemaEvolutionManager:
    ...
    """
    def __init__(self, current_schema_path: str = "conf/features_params.yaml",
    ...
    def compare_schemas(self, old_schema: Dict[str, Any], new_schema: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for feature in new_features - old_features:
            differences["added_features"].append({"name": feature, "definition": new_schema[feature]})

        # Removed features
        for feature in old_features - new_features:
            differences["removed_features"].append({"name": feature, "definition": old_schema[feature]})
        ...
        for feature in old_features.intersection(new_features):
            ...
            for key, new_value in new_def.items():
                if key not in old_def:
                    feature_diff[key] = {"old": "added", "new": new_value} # New property in existing feature

            if feature_diff:
                differences["modified_features"][feature] = feature_diff
        
        logging.info(f"Schema comparison complete. Found {len(differences['added_features'])} added, "
                     f"{len(differences['removed_features'])} removed, and {len(differences['modified_features'])} modified features.")
        return differences

    def assess_compatibility(self, schema_diff: Dict[str, Any]) -> Tuple[bool, List[str]]:
        """
        ...
        for removed_feature in schema_diff["removed_features"]:
            if removed_feature["definition"].get("required"):
                breaking_changes.append(f"Removed required feature: '{removed_feature['name']}'")
                is_compatible = False
        ...
        for feature, changes in schema_diff["modified_features"].items():
            if "type" in changes:
                ...
                if old_type_def != "added" and new_type_def != "removed": # Not a newly added/removed property
                    # If feature was required, changing its type is a breaking change
                    old_required = self._get_required_status(feature, schema_diff.get("old_schema_snapshot", {}))
                    new_required = self._get_required_status(feature, schema_diff.get("new_schema_snapshot", {}))

                    if old_required or new_required: # If it was or became required, type change is critical
                        breaking_changes.append(f"Type changed for required/critical feature '{feature}': from {old_type_def} to {new_type_def}")
                        is_compatible = False
        
        if not is_compatible:
            logging.error(f"Schema changes are NOT backward compatible: {breaking_changes}")
        else:
...
if __name__ == '__main__':
    ...
    base_schema = {
        "geohash_id": {"type": "string", "required": True, "description": "Unique identifier for the geographic area"},
        "hour_of_day": {"type": "integer", "required": True, "min_value": 0, "max_value": 23},
        "demand_current_5min": {"type": "number", "required": True, "min_value": 0.0},
        "is_weekend": {"type": "integer", "allowed_values": [0, 1]},
        "temperature_celsius": {"type": "number", "min_value": -50, "max_value": 50, "required": False}
    }

    # Scenario 1: Backward-compatible change (add optional feature)
    new_schema_compatible = base_schema.copy()
    new_schema_compatible["precipitation_mm"] = {"type": "number", "min_value": 0.0, "required": False}
    new_schema_compatible["hour_of_day"] = {"type": "integer", "required": True, "min_value": 0, "max_value": 23, "unit": "hours"} # Added a property

    print("--- Scenario 1: Compatible Change (Add optional feature, add property to existing) ---")
    ...
    diff_break_removed = manager.compare_schemas(base_schema, new_schema_breaking_removed)
    ...
    new_schema_breaking_type["hour_of_day"] = {"type": "string", "required": True, "allowed_values": ["morning", "afternoon", "evening"]}
