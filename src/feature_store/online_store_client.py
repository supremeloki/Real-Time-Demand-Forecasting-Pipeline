...
class OnlineFeatureStoreClient:
    def __init__(self, feature_source_path="data_artifacts/sample_data/batch_processed_features.csv"):
        ...
        try:
            ...
            if 'event_timestamp' in self.historical_features.columns:
                self.historical_features['timestamp'] = pd.to_datetime(self.historical_features['event_timestamp'])
            elif 'timestamp' not in self.historical_features.columns:
                # If no timestamp column, create a mock one
                self.historical_features['timestamp'] = pd.date_range(start=datetime.now() - timedelta(days=1), periods=len(self.historical_features), freq='15min')
            self.historical_features['timestamp'] = pd.to_datetime(self.historical_features['timestamp'])
            self.historical_features = self.historical_features.set_index('timestamp').sort_index()
    ...
    def get_latest_features(self, geohash_id: str, current_time: datetime) -> dict:
        ...
        mock_features = {
            ...
            "peak_hour_indicator": 1 if 7 <= current_time.hour < 10 or 16 <= current_time.hour < 19 else 0,
