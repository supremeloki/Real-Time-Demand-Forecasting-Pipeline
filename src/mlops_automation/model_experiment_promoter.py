...
class ModelExperimentPromoter:
    ...
    def __init__(self, mlflow_tracking_uri: str = "http://localhost:5000"):
        ...
        logging.info(f"ModelExperimentPromoter initialized with MLflow tracking: {mlflow_tracking_uri}")
    ...
    def _evaluate_candidate_model(self, candidate_model_uri: str, production_model_uri: str) -> bool:
        """
        ...
        if candidate_rmse < prod_rmse * 0.95: # Candidate must be significantly better
            ...
            logging.info("Candidate model shows slight improvement, consider for A/B testing.")
    ...
    def promote_to_production(self, model_name: str, version: int):
        ...
        for mv in self.mlflow_client.search_model_versions(f"name='{model_name}'"):
            if mv.current_stage == "Production":
                logging.info(f"Archiving current Production model {model_name} version {mv.version}.")
                self.mlflow_client.transition_model_version_stage(
                    name=model_name,
                    version=mv.version,
                    stage="Archived"
        ...
        logging.info(f"Model {model_name} version {version} transitioned to Production.")

    def update_ab_test_config(self, experiment_id: str, traffic_split: Dict[str, float]):
        """
        ...
        """
        logging.info(f"Updating A/B test '{experiment_id}' with traffic split: {traffic_split}")
        # Placeholder for actual config update (e.g., to a Redis cache or ConfigMap)
        self.ab_test_configs[experiment_id] = {"traffic_split": traffic_split, "last_updated": datetime.now().isoformat()}
...
if __name__ == '__main__':
    ...
    promoter = ModelExperimentPromoter(mlflow_tracking_uri="sqlite:///mlruns.db") # Use local path for demo
    ...
    candidate_version = promoter.mlflow_client.search_model_versions(f"name='{model_name}'")[-1].version
    ...
    try:
        current_prod_version_obj = [mv for mv in promoter.mlflow_client.search_model_versions(f"name='{model_name}'") if mv.current_stage == "Production"]
        if not current_prod_version_obj:
            # Manually create a dummy production version for demo if none exists
            logging.warning("No Production model found. Creating a dummy one for comparison.")
            promoter.mlflow_client.transition_model_version_stage(
                name=model_name,
                version=candidate_version, # Promote the first candidate as dummy prod
                stage="Production"
            )
            promoter.mlflow_client.log_metric(run_id=promoter.mlflow_client.get_model_version(name=model_name, version=candidate_version).run_id, key="val_rmse", value=10.0)
            promoter.mlflow_client.log_metric(run_id=promoter.mlflow_client.get_model_version(name=model_name, version=candidate_version).run_id, key="val_mae", value=7.0)
        ...
        is_better = promoter._evaluate_candidate_model(candidate_model_uri, current_prod_model_uri)
        if is_better:
        ...
        logging.error(f"Error in main block: {e}. Please ensure MLflow is running and some models exist.")
