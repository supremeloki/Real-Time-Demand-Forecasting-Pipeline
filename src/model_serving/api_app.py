...
@app.on_event(
    "startup"
)  # TODO: Consider migrating to lifespan events in future FastAPI version
...
class SinglePredictionResponse(BaseModel):
    predicted_demand: float = Field(
        ..., description="Predicted number of rides in the next 15 minutes."
    )
...
async def health_check():
    return {
        ...
        "message": (
            ...
            if prediction_logic_instance
            else "API is healthy, but model loading failed."
        ),
    }
