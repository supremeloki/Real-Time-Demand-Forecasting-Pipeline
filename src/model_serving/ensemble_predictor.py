...
class EnsemblePredictor:
    def __init__(self, model_clients: List[Any], weights: List[float] = None):
        ...
        self.weights = (
            weights if weights else [1.0 / len(model_clients)] * len(model_clients)
        )
        if len(self.weights) != len(self.model_clients):
            raise ValueError("Number of weights must match number of model clients.")
        if not np.isclose(sum(self.weights), 1.0):
            logging.warning(
                "Ensemble weights do not sum to 1.0. They will be normalized."
            )
            self.weights = [w / sum(self.weights) for w in self.weights]

        logging.info(
            f"EnsemblePredictor initialized with {len(model_clients)} models and weights: {self.weights}"
        )
    ...
    def predict(self, features: Dict[str, Any]) -> float:
        ...
        for i, client in enumerate(self.model_clients):
            try:
                ...
                logging.error(
                    f"Error getting prediction from model client {i+1}: {e}. Skipping this model."
                )
        ...
        valid_predictions = [
            pred for pred in individual_predictions if pred is not None
        ]
        if len(valid_predictions) != len(individual_predictions):
            logging.warning(
                f"Some models failed. Re-normalizing weights for {len(valid_predictions)} successful models."
            )
            ...
            ensemble_prediction = np.average(
                individual_predictions, weights=self.weights
            )
...
if __name__ == "__main__":
    ...
    class MockModelClient:
        ...
        def predict(self, features: Dict[str, Any]) -> float:
            ...
            if np.random.rand() < 0.05:
                raise RuntimeError(
                    f"MockModelClient {self.model_id} simulated failure."
                )
    ...
    sample_features = {"hour_of_day": 9, "demand_current_5min": 15.0, "is_weekend": 0}
