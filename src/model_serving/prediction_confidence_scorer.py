...
class PredictionConfidenceScorer:
    ...
    def __init__(
        self, historical_error_std: float = 5.0, confidence_threshold: float = 0.7
    ):
        self.historical_error_std = historical_error_std
        self.confidence_threshold = confidence_threshold
        logging.info(
            f"PredictionConfidenceScorer initialized with historical_error_std={historical_error_std}"
        )

    def score_ensemble_confidence(
        self, individual_predictions: List[float]
    ) -> Dict[str, Any]:
        """
        ...
        if not individual_predictions or len(individual_predictions) < 2:
            logging.warning(
                "Insufficient individual predictions for ensemble confidence scoring. Returning default."
            )
            return {
        ...
        normalized_spread = prediction_spread / (
            self.historical_error_std * 2
        )  # Factor of 2 for reasonable scale
        confidence_score = max(
            0.0, 1.0 - min(1.0, normalized_spread)
        ...
        if confidence_score >= self.confidence_threshold:
            ...
            logging.warning(
                f"Low confidence prediction detected. Spread: {prediction_spread:.2f}, Score: {confidence_score:.2f}"
            )
    ...
    def score_model_confidence_single(
        self, prediction: float, recent_errors: List[float]
    ) -> Dict[str, Any]:
        """
        ...
        """
        if (
            not recent_errors or len(recent_errors) < 5
        ):  # Need a few errors to estimate variance
            logging.warning(
                "Insufficient recent errors for single model confidence scoring. Returning default."
            )
            return {
                "confidence_score": 0.5,
                "confidence_level": "MEDIUM",
                "error_variability": self.historical_error_std,
            }
        ...
        confidence_score = max(
            0.0, 1.0 - min(1.0, variability_ratio - 0.5)
        )  # Adjust for reasonable scaling
        ...
        if confidence_score >= self.confidence_threshold:
            ...
            confidence_level = "LOW"
            logging.warning(
                f"Low confidence prediction detected. Error variability: {current_error_std:.2f}, Score: {confidence_score:.2f}"
            )
...
if __name__ == "__main__":
    scorer = PredictionConfidenceScorer(
        historical_error_std=7.0, confidence_threshold=0.8
    )
    ...
    conf_single_high = scorer.score_model_confidence_single(
        prediction=50.0, recent_errors=recent_errs_high_conf
    )
    ...
    conf_single_low = scorer.score_model_confidence_single(
        prediction=50.0, recent_errors=recent_errs_low_conf
    )
