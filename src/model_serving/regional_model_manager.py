...
class RegionalModelManager:
    ...
    def _load_initial_models(self):
        ...
        self.loaded_models[self.default_model_name] = self._get_model_from_registry(self.default_model_name)

        # Load regional models
        for region_prefix, model_name in self.region_model_map.items():
            if model_name not in self.loaded_models: # Avoid re-loading if same as default or already loaded
                logging.info(f"Loading regional model for {region_prefix}: {model_name}")
                self.loaded_models[model_name] = self._get_model_from_registry(model_name)
        ...
        try:
            ...
            class MockModel:
                ...
                def predict(self, features: Dict[str, Any]) -> float:
                    ...
                    return features.get('demand_current_5min', 10.0) * random.uniform(3, 5) + (5 if "local" in self.name else 0)
            
            logging.info(f"Mock-loaded model: {model_name}")
            return MockModel(model_name)
        except Exception as e:
            logging.error(f"Failed to load model '{model_name}' from registry: {e}. Returning None.")
            return None
    ...
    def get_model_for_geohash(self, geohash_id: str) -> Any:
        ...
        for region_prefix, model_name in self.region_model_map.items():
            if geohash_id.startswith(region_prefix):
                ...
                if model:
                    logging.debug(f"Using regional model '{model_name}' for geohash '{geohash_id}'.")
                    return model
                else:
                    logging.warning(f"Regional model '{model_name}' for '{geohash_id}' not found/loaded. Falling back to default.")
        ...
        logging.debug(f"Using default model '{self.default_model_name}' for geohash '{geohash_id}'.")
