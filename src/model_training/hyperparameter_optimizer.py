...
class HyperparameterOptimizer:
    def __init__(
        ...
        model_trainer: Callable[
            [Dict[str, Any]], float
        ],  # A function that trains a model and returns a metric
        ...
        if "MLFLOW_TRACKING_URI" not in os.environ:
            logging.warning(
                "MLFLOW_TRACKING_URI environment variable not set. Using default local MLflow tracking."
            )
            mlflow.set_tracking_uri("sqlite:///mlruns.db")  # Default for local demo

        mlflow.set_experiment(experiment_name)
        logging.info(
            f"HyperparameterOptimizer initialized for experiment '{experiment_name}'."
        )
    ...
    def _objective(self, trial: optuna.Trial) -> float:
        ...
        with mlflow.start_run(run_name=f"trial_{trial.number}"):
            ...
            metric_value = self.model_trainer(
                params
            )  # Call the external trainer function
    ...
    def run_optimization(self) -> Dict[str, Any]:
        ...
        """
        logging.info(
            f"Starting hyperparameter optimization with {self.num_trials} trials..."
        )
        study = optuna.create_study(
            direction="minimize", study_name=self.experiment_name
        )
...
if __name__ == "__main__":
    ...
    def mock_xgboost_trainer(params: Dict[str, Any]) -> float:
        ...
        if params.get("n_estimators", 100) > 150:
            rmse *= 0.9  # Better n_estimators
        if params.get("learning_rate", 0.1) < 0.05:
            rmse *= 1.1  # Worse learning rate
