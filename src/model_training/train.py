...
class ModelTrainer:
    ...
    def _get_feature_columns(self):
        ...
        demand_hist_feats = self.config.get("features_params.demand_history_features", [])
        ...
        all_features = list(set(time_feats + location_feats + weather_feats + demand_hist_feats))
        
        # Manually ensure some are always included and others are removed if they are only for aggregation
        if 'grid_id_latitude' in all_features: all_features.remove('grid_id_latitude')
        if 'grid_id_longitude' in all_features: all_features.remove('grid_id_longitude')
        ...
        return [f for f in generated_features if f != self.target_variable] # Remove target if present
    ...
    def train_model(self):
        ...
        with mlflow.start_run(run_name="Demand_Forecaster_XGBoost") as run:
            ...
            logger.info(f"Model trained and logged to MLflow as {self.model_name}, version {mlflow.active_run().info.run_id}")
