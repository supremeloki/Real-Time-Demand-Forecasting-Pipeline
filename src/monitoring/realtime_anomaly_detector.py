...
class RealtimeAnomalyDetector:
    def __init__(self, contamination: float = 0.05, window_size: int = 100):
        ...
        self.data_buffer = []
        logging.info(
            f"Initialized RealtimeAnomalyDetector with contamination={contamination} and window_size={window_size}"
        )
    ...
    def detect_anomaly(self, current_features: dict) -> bool:
        ...
        try:
            ...
            if is_anomaly:
                logging.warning(
                    f"Anomaly detected! Current demand: {current_demand} at {datetime.now()}"
                )
            else:
...
if __name__ == "__main__":
    ...
    for i in range(120):  # Simulate 120 data points
        mock_features = {"demand_last_15min": np.random.normal(loc=20, scale=5)}
