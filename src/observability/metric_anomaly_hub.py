...
class MetricAnomalyHub:
    ...
    def __init__(self, alert_threshold: float = 0.8):
        ...
        self.registered_detectors: Dict[str, Any] = (
            {}
        )  # e.g., {'data_drift': ConceptDriftDetector_instance}
    ...
    def ingest_metrics_and_check_anomalies(
        self, metric_stream: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for detector_name, detector in self.registered_detectors.items():
            try:
                ...
                if detector_name == "demand_forecast_drift":
                    ...
                    if (
                        cpu_util is not None and cpu_util > 0.9
                    ):  # Simple rule-based spike
                        is_anomaly = True
                        anomaly_details = {
                ...
                if is_anomaly:
                    anomaly_record = {
                        ...
                        "alert_raised": anomaly_details.get("severity", 0)
                        >= self.alert_threshold,
                    }
                    detected_anomalies.append(anomaly_record)
                    self.anomaly_history.append(anomaly_record)
                    logging.error(
                        f"ANOMALY DETECTED by '{detector_name}': {anomaly_record}"
                    )
...
if __name__ == "__main__":
    ...
    class MockResourceDetector:
        def check_spike(self, cpu_util):
            return False

    class MockModelConfidenceDetector:
        def score_confidence(self, preds):
            return {"confidence_level": "HIGH", "confidence_score": 0.9}
    ...
    for i in range(20):
        ...
        if anomalies:
            for anomaly in anomalies:
                print(
                    f"  Anomaly: {anomaly['detector']}, Alert: {anomaly['alert_raised']}, Details: {anomaly['details']}"
                )
        else:
