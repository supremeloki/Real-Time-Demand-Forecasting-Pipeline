...
class DriverAllocationOptimizer:
    def __init__(self, demand_forecaster_client):
        self.demand_forecaster_client = demand_forecaster_client # Client to interact with prediction API
    ...
    def _get_active_drivers(self, geohash_id: str) -> int:
        ...
        self.active_drivers_per_geohash[geohash_id] = max(1, self.active_drivers_per_geohash[geohash_id])
        return self.active_drivers_per_geohash[geohash_id]

    def _calculate_demand_supply_gap(self, predicted_demand: float, active_drivers: int) -> float:
    ...
    def optimize_allocation_for_geohash(self, geohash_id: str, forecast_horizon_mins: int = 15) -> dict:
        ...
        mock_features = {
            ...
            "is_holiday": 0,
            "peak_hour_indicator": 1 if 7 <= datetime.now().hour < 10 or 16 <= datetime.now().hour < 19 else 0,
        ...
        logging.info(f"Predicted demand for {geohash_id} in next {forecast_horizon_mins} mins: {predicted_demand:.2f}")
        ...
        demand_supply_gap_score = self._calculate_demand_supply_gap(predicted_demand, active_drivers)
        ...
        if demand_supply_gap_score > 2.0: # High demand, low supply
            ...
            logging.warning(f"High demand-supply gap in {geohash_id}. Recommend increasing drivers.")
            ...
            incentive_level = "LOW"
            logging.info(f"Low demand-supply gap in {geohash_id}. Optimize existing drivers.")
        else:
            recommendation = "MAINTAIN_CURRENT"
            target_drivers = active_drivers
            incentive_level = "MEDIUM"
            logging.info(f"Balanced demand-supply in {geohash_id}. Maintain current state.")
...
if __name__ == '__main__':
    ...
    class MockDemandForecasterClient:
        def predict(self, geohash_id: str, features: dict) -> float:
            ...
            return features['demand_current_5min'] * random.uniform(3, 5) # Scale 5-min demand to 15-min forecast
    ...
    for _ in range(3): # Run a few rounds to see changes
        for gh in geohashes_to_optimize:
            ...
            print(f"  Recommendation: {result['recommendation']} (Target: {result['target_drivers']} drivers, Incentive: {result['incentive_level']})")
        time.sleep(1)