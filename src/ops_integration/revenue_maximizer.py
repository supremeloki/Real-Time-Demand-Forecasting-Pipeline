...
class RevenueMaximizer:
    ...
    def __init__(self, base_ride_fare: float = 5.0):
        ...
        logging.info(f"RevenueMaximizer initialized with base_ride_fare={base_ride_fare}")

    def _estimate_completed_rides(self, predicted_demand: float, active_drivers: int, driver_capacity_per_hour: float = 3.0) -> float:
        """
    ...
    def calculate_projected_revenue(self,
                                    ...
                                    forecast_horizon_hours: float = 1.0) -> Dict[str, Any]:
        """
        Calculates projected revenue for a specific geohash and forecast horizon.
        """
        logging.info(f"Calculating revenue for {geohash_id}: Demand={predicted_demand:.2f}, Drivers={active_drivers}, Multiplier={pricing_multiplier:.2f}")
        ...
        scaled_demand = predicted_demand * (forecast_horizon_hours / 0.25) # 0.25 hours = 15 minutes

        estimated_completed_rides = self._estimate_completed_rides(scaled_demand, active_drivers)
        
        projected_revenue = estimated_completed_rides * self.base_ride_fare * pricing_multiplier
        
        # Determine operational insight based on the gap between demand and completed rides
        demand_completion_ratio = estimated_completed_rides / scaled_demand if scaled_demand > 0 else 1.0
        
        operational_insight = "OPTIMAL_BALANCE"
        if demand_completion_ratio < 0.7:
            operational_insight = "DEMAND_SURPLUS_UNDER_STAFFED"
        elif demand_completion_ratio > 1.2: # Implies more supply than demand (simplified)
            operational_insight = "SUPPLY_SURPLUS_OVER_STAFFED"

        logging.info(f"Projected revenue for {geohash_id}: {projected_revenue:.2f} (Completed Rides: {estimated_completed_rides:.1f})")
...
if __name__ == '__main__':
    ...
    for _ in range(3): # Simulate a few time steps
        ...
        for gh in geohashes_to_analyze:
            ...
            print(f"  {revenue_analysis['geohash_id']}: Demand={revenue_analysis['predicted_demand_scaled']:.1f} (1hr), Drivers={revenue_analysis['active_drivers']}, Multiplier={revenue_analysis['pricing_multiplier']:.2f}")
            print(f"    -> Est. Rides={revenue_analysis['estimated_completed_rides']:.1f}, Projected Revenue=${revenue_analysis['projected_revenue']:.2f}, Insight: {revenue_analysis['operational_insight']}")
        # time.sleep(0.5)