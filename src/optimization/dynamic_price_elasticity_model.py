...
class DynamicPriceElasticityModel:
    ...
    def __init__(self,
        ...
        self.historical_data: Dict[str, List[Dict[str, Any]]] = {} # Store (price_multiplier, demand, completed_rides)
        logging.info(f"DynamicPriceElasticityModel initialized with initial elasticity: {initial_elasticity}")
    ...
    def update_elasticity(self,
                          ...
                          observed_demand_change: float): # Percentage change in demand (e.g., 0.1 for 10% increase)
        """
        ...
        price_change_ratio = (new_price_multiplier - old_price_multiplier) / old_price_multiplier
        if price_change_ratio == 0:
            logging.debug("No price change, skipping elasticity update.")
            return
        
        # Calculate observed elasticity
        observed_elasticity = observed_demand_change / price_change_ratio if price_change_ratio != 0 else self.initial_elasticity
        
        # Simple exponential moving average (EMA) update
        current_eta = self._get_current_elasticity(geohash_id)
        new_eta = current_eta * (1 - self.learning_rate) + observed_elasticity * self.learning_rate
        ...
        logging.info(f"Elasticity for {geohash_id} updated: {current_eta:.2f} -> {new_eta:.2f} (Observed: {observed_elasticity:.2f})")

    def predict_demand_impact(self,
                              geohash_id: str,
                              current_demand: float,
                              price_multiplier_change_percentage: float) -> float:
        """
        ...
        logging.debug(f"Predicted demand change for {geohash_id} with {price_multiplier_change_percentage*100:.1f}% price change: {predicted_demand_change*100:.1f}%")
        return current_demand * (1 + predicted_demand_change)

if __name__ == '__main__':
    elasticity_model = DynamicPriceElasticityModel(initial_elasticity=-0.7, learning_rate=0.1)
    ...
    observed_demand_change_percent_1 = (observed_demand_1 - initial_demand) / initial_demand
    
    print(f"Price increased to {proposed_price_multiplier_1:.1f}. Observed demand: {observed_demand_1:.1f}")
    ...
    print(f"Updated Elasticity for {geohash}: {elasticity_model._get_current_elasticity(geohash):.2f}")
    
    predicted_demand_after_10_percent_inc = elasticity_model.predict_demand_impact(geohash, observed_demand_1, 0.1)
    print(f"If price increases by 10% from current, predicted demand: {predicted_demand_after_10_percent_inc:.1f}")
    ...
    observed_demand_change_percent_2 = (observed_demand_2 - current_demand_2) / current_demand_2

    print(f"Price decreased to {proposed_price_multiplier_2:.1f}. Observed demand: {observed_demand_2:.1f}")
    elasticity_model.update_elasticity(
        geohash_id=geohash,
