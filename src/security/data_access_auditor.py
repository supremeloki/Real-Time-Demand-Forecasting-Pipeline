...
class DataAccessAuditor:
    ...
    def __init__(
        self,
        audit_log_storage_path: str = "/var/log/snapp_ml/data_access.log",
        ...
        logging.info(
            f"DataAccessAuditor initialized. Logging to {audit_log_storage_path}"
        )
    ...
    def _log_audit_event(self, event: Dict[str, Any]):
        ...
        try:
            ...
            logging.error(
                f"Failed to write audit log to {self.audit_log_storage_path}: {e}"
            )
    ...
    def audit_access(
        ...
        if not is_successful:
            self.failed_attempts_by_user[user_id] = (
                self.failed_attempts_by_user.get(user_id, 0) + 1
            )
            if self.failed_attempts_by_user[user_id] >= self.alert_on_failed_attempts:
                logging.critical(
                    f"SECURITY ALERT: User '{user_id}' has too many failed access attempts "
...
if __name__ == "__main__":
    auditor = DataAccessAuditor(
        audit_log_storage_path="/tmp/snapp_ml_access.log", alert_on_failed_attempts=3
    )

    # Ensure the log file is clean for a fresh run
    with open("/tmp/snapp_ml_access.log", "w") as f:
        pass
    ...
    auditor.audit_access(
        "admin_user", "model_registry", "DemandForecaster/2", "read", True
    )
