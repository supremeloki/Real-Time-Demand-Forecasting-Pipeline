...
class ThreatIntelligenceReactor:
    def __init__(
        self, incident_response_endpoint: str = "http://incident-manager:8080/alert"
    ):
    ...
    def _trigger_containment(self, threat_level: str, affected_system: str):
        if threat_level == "CRITICAL":
            print(
                f"[REACTOR ACTION] Initiating automatic containment for {affected_system}!"
            )
        elif threat_level == "HIGH":
            print(
                f"[REACTOR ACTION] Escalating to human intervention for {affected_system}."
            )
