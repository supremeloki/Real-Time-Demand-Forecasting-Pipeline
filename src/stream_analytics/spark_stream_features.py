...
class SparkStreamFeatures:
    ...
    def calculate_grid_id(self, latitude, longitude):
        grid_lat = (latitude // (self.grid_size_km / 111.0)) * (self.grid_size_km / 111.0) # Approx 111km per degree lat
        grid_lon = (longitude // (self.grid_size_km / (111.0 * col("cos_lat")))) * (self.grid_size_km / (111.0 * col("cos_lat")))
    ...
    def process_stream(self):
        ...
        parsed_stream_df = kafka_stream_df.selectExpr("CAST(value AS STRING) as json_value") \
            .withColumn("data", from_json(col("json_value"), schema)) \
            .select("data.*") \
            .withColumn("timestamp", to_timestamp(col("event_timestamp")))
        
        # Calculate grid IDs
        parsed_stream_df = parsed_stream_df.withColumn("cos_lat", (col("pickup_latitude") * (3.14159 / 180.0)).cos())
        parsed_stream_df = parsed_stream_df.withColumn("grid_id_latitude", (col("pickup_latitude") / (self.grid_size_km / 111.0)).cast("int"))
        parsed_stream_df = parsed_stream_df.withColumn("grid_id_longitude", (col("pickup_longitude") / (self.grid_size_km / (111.0 * col("cos_lat")))).cast("int"))

        # Aggregate demand per grid and time window
        demand_features_df = parsed_stream_df \
        ...
        demand_features_df = demand_features_df.withColumn("current_time", current_timestamp())
        for window_size in self.lookback_windows_min:
            demand_features_df = demand_features_df.withColumn(
                ...
                (demand_features_df["demand_sum"] * (60 / window_size)) # Placeholder, actual logic would be complex stateful aggregation
            )

        # Rename columns to match expected feature names
        demand_features_df = demand_features_df.withColumnRenamed("demand_sum", "demand_last_15min")
        demand_features_df = demand_features_df.withColumnRenamed("demand_last_30min", "demand_last_30min")
        demand_features_df = demand_features_df.withColumnRenamed("demand_last_60min", "demand_last_60min")
        demand_features_df = demand_features_df.withColumnRenamed("demand_last_1440min", "demand_last_1440min")

        output_df = demand_features_df \
            .select(
                ...
                col("demand_last_1440min") # Corresponds to 'demand_same_hour_last_day'
            ) \
            .withColumn("value", col("struct(*)")).drop("struct(*)")

        query = output_df.selectExpr("CAST(grid_id_latitude AS STRING) as key", "to_json(struct(*)) AS value") \
            .writeStream \
            .format("kafka") \
            .option("kafka.bootstrap.servers", self.kafka_bootstrap_servers) \
        ...
        logger.info(f"Spark Structured Streaming started for topic {self.kafka_input_topic}. Writing to {self.kafka_output_topic}")
